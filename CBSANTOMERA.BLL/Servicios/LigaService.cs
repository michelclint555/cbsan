...
namespace CBSANTOMERA.BLL.Servicios
{
    public class LigaService : ILigaService
    {
        ...
        public LigaService(IGenericRepository<Liga> repository)
        {
            _repository = repository;
        }

        public async Task IniciarLigaAsync(
            int competicionId,
            int faseLigaId,
            List<int> equiposIds)
        {
            // 1️⃣ Validaciones
            if (equiposIds == null || equiposIds.Count < 3)
                throw new Exception("No hay suficientes equipos para iniciar la liga");

            // 2️⃣ Evitar reinicio
            var existentes = await _repository.Consultar(l => l.Fase == faseLigaId);
            if (existentes.Any())
                throw new Exception("La liga ya está iniciada");

            // 3️⃣ Crear clasificación
            foreach (var equipoId in equiposIds)
            {
                await _repository.Crear(new Liga
                {
            ...
            // 4️⃣ Posición inicial
            await ReordenarClasificacionAsync(faseLigaId);
        }



        public async Task ProcesarPartidoFinalizadoAsync(
      Partido partido,
      int faseId)
        {
            ...
            await ReordenarClasificacionAsync(faseId);
        }
    ...
    bool esLocal)
        {
            var liga = await _repository.ObtenerUnModelo(l =>
                l.Fase == faseId && l.Equipo == equipoId);

            if (liga == null)
                return;

            int puntosFavor = esLocal ? partido.PuntosLocal : partido.PuntosVisitante;
            int puntosContra = esLocal ? partido.PuntosVisitante : partido.PuntosLocal;

            liga.PartidosJugados++;
            liga.DifetenciaPuntos += puntosFavor - puntosContra;

            if (puntosFavor > puntosContra)
            {
                liga.PartidosGanados++;
                liga.Puntos += 2;
                liga.Racha = (liga.Racha ?? 0) + 1;
            }
            else
            {
                liga.PartidosPerdidos++;
                liga.Racha = 0;
            }

            await _repository.Editar(liga);
        }
        private async Task ReordenarClasificacionAsync(int faseId)
        {
            var clasificacion = await _repository.Consultar(l => l.Fase == faseId);

            var ordenada = clasificacion
                .OrderByDescending(l => l.Puntos)
                .ThenByDescending(l => l.DifetenciaPuntos)
                .ToList();

            int posicion = 1;
            foreach (var fila in ordenada)
            {
                fila.Posicion = posicion++;
                await _repository.Editar(fila);
            }
        }
        public async Task<List<LigaDTO>> ObtenerClasificacionAsync(int faseId)
        {
            var clasificacion = await _repository.Consultar(l => l.Fase == faseId);

            return clasificacion
                .OrderBy(l => l.Posicion)
                .Select(LigaDTO.ToDTO)
                .ToList();
        }
        ...
        /*    public async Task<LigaDTO> Crear(LigaDTO modelo)
            ...
            public List<List<JornadaDTO>> ExtraerCalendarioInicial(string ruta, List<EquipoCompeticionDTO> equipos, CompeticionDTO competicion, FaseCompeticionDTO fase)
            {
                ...
                foreach (var page in doc.GetPages())
                {
                  ...
                  //  Regex regex = new Regex(@"([A-ZÑÁÉÍÓÚ0-9\s\""]{5,})\s+(Murcia|Cartagena|San Javier|Santomera|Los Dolores)", RegexOptions.Multiline);
                    ...
                    try
                    {
                        var jornadas1 = ParserFBRM.Parse(texto);
                        ...
                        Console.WriteLine($"\nTotal partidos parseados: {jornadas.Count}");
                    }
                    catch (Exception ex)
                    {
                    ...
                    // Regex para capturar cabecera de jornada (fecha + número)
                    Regex regexJornada = new Regex(@"(\d{2}/\d{2}/\d{4}).*?JORNADA.*?(\d+)", RegexOptions.IgnoreCase);

                    // Regex para capturar partidos (EquipoLocal EquipoVisitante PuntosLocal PuntosVisitante)
                    Regex regexPartido = new Regex(@"([A-Z0-9\.\s\(\)]+?)\s+([A-Z0-9\.\s\(\)]+?)\s+(\d{2,3})\s+(\d{2,3})");
                    ...
                    {
                        var matchJornada = regexJornada.Match(linea);
                        if (matchJornada.Success)
                        {
                            fechaJornada = DateTime.Parse(matchJornada.Groups[1].Value);
                            numeroJornada++;
                            continue;
                        }

                        var matchPartido = regexPartido.Match(linea);
                        if (matchPartido.Success)
                        {

                        string Local = matchPartido.Groups[1].Value.Trim();
                        string Visitante = matchPartido.Groups[2].Value.Trim();

                        int PuntosLocal = int.Parse(matchPartido.Groups[3].Value);
                      int PuntosVisitante = int.Parse(matchPartido.Groups[4].Value);
                        }
                    }
            ...
            static void ExtraerEquiposDesdeCadena(string texto, List<EquipoCompeticionDTO> equipos, int jornada)
            {
           ...
           /*     List<JornadaDTO> lista = new List<JornadaDTO>();
            ...
            public static void ExtraerSubcadenaEntreEquipos(string texto, EquipoCompeticionDTO equipoInicio, EquipoCompeticionDTO equipoFin, List<EquipoCompeticionDTO> equipos)
            {
    ...
    /*
            ...
            public static void ExtraerSubcadenaEntreEquipos0(string texto, EquipoCompeticionDTO equipoInicio, EquipoCompeticionDTO equipoFin, List<EquipoCompeticionDTO> equipos)
            {
            ...
            public async void Editar(JornadaDTOSimple modelo)
            {
    ...
    public static class ParserFBRM
    {
        private static readonly string[] Equipos = new[]
        {
            "GARGIL SUMINISTROS MB",
            "INFANTE",
            "CB. MAJONA BENIEL ALQUERIAS",
            "UCAM MURCIA ( JUNIOR)",
            "C.B. SEGUROS ROCAMORA BEGASTRI",
            "ODILO FC CARTAGENA CB",
            "ECONEX C.B. SANTOMERA",
            "ACEITUNAS FRUYPER C.B. SAN JOSÉ"
        };

        private static readonly Dictionary<string, string> PabellonPorLocal = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ["GARGIL SUMINISTROS MB"] = "Pabellón Serrerías (Molina de Segura)",
            ["INFANTE"] = "Pabellón Federico Aliaga (Infante, Murcia)",
            ["CB. MAJONA BENIEL ALQUERIAS"] = "Pabellón Carlos Ferrer (Beniel)",
            ["UCAM MURCIA ( JUNIOR)"] = "Pabellón Narciso Yepes (Murcia)",
            ["C.B. SEGUROS ROCAMORA BEGASTRI"] = "Pabellón Ana Carrasco (Cehegín)",
            ["ODILO FC CARTAGENA CB"] = "Pabellón Los Dolores (Cartagena)",
            ["ECONEX C.B. SANTOMERA"] = "Pabellón Municipal (Santomera)",
            ["ACEITUNAS FRUYPER C.B. SAN JOSÉ"] = "Pabellón Juan Martínez Marín (San José de la Vega)"
        };
        ...
        // 🔹 Buscar el equipo más parecido a un fragmento
        private static string Normalizar(string s)
        {
            return Regex.Replace(s.ToUpper(), @"[^A-Z0-9]", "");
        }
        private static (string, string) DetectarEquipo(string bloque)
        {
            // Buscar los dos equipos más parecidos en la lista oficial
            var encontrados = Equipos
                .Select(eq => new { Eq = eq, Dist = Levenshtein(Normalizar(bloque), Normalizar(eq)) })
                .OrderBy(x => x.Dist)
                .Take(2)
                .Select(x => x.Eq)
                .ToList();
        ...
        private static string DetectarParejaEquipos(string fragmento)
        {
            string norm = Normalizar(fragmento);
            string mejorEquipo = null;
            int mejorDistancia = int.MaxValue;

            foreach (var eq in Equipos)
            {
                string eqNorm = Normalizar(eq);
                int dist = Levenshtein(norm, eqNorm);
            ...
            // Opcional: descartar si es muy distinto (> 8 caracteres de distancia, por ejemplo)
            if (mejorDistancia > 8)
                throw new Exception($"No se pudo emparejar '{fragmento}' con ningún equipo (distancia mínima {mejorDistancia}).");

            return mejorEquipo;
        }

        // 🔹 Separar los dos marcadores concatenados (ej: 8863 -> 88 y 63)
        private static (int, int) SepararMarcadores(string marcador)
        {
            if (marcador.Length == 4) // 88 63
                return (int.Parse(marcador.Substring(0, 2)), int.Parse(marcador.Substring(2, 2)));
            if (marcador.Length == 6) // 111 103
                return (int.Parse(marcador.Substring(0, 3)), int.Parse(marcador.Substring(3, 3)));

            throw new Exception($"Formato de marcador no reconocido: {marcador}");
        }

        public static List<Jornada0> Parse(string texto)
        {
            var jornadas = new List<Jornada0>();

            // Regex para extraer cada jornada completa
            var rxJornada = new Regex(
                @"(?<fecha>\d{2}/\d{2}/\d{4}).*?JORNADA.*?(?<num>\d+)(?<body>.*?)(?=\d{2}/\d{2}/\d{4}.*?JORNADA|$)",
                RegexOptions.Singleline);

            foreach (Match m in rxJornada.Matches(texto))
            {
                DateTime fecha = DateTime.ParseExact(m.Groups["fecha"].Value, "dd/MM/yyyy", CultureInfo.InvariantCulture);
                int numero = int.Parse(m.Groups["num"].Value);
                string body = m.Groups["body"].Value;

                // Buscar partidos dentro de la jornada
                var rxPartido = new Regex(@"([A-Z0-9\.\(\) ]+?)([A-Z0-9\.\(\) ]+?)(\d{4,6})", RegexOptions.Singleline);
                foreach (Match p in rxPartido.Matches(body))
                {
                    var (local, visitante)= DetectarEquipo(p.Groups[2].Value);
                    //string visitante = DetectarEquipo(p.Groups[2].Value);
                    //(int pl, int pv) = SepararMarcadores(p.Groups[3].Value);

                    jornadas.Add(new Jornada0
                    {
                        NumeroJornada = numero,
                        Local = local,
                        Visitante = visitante,
                        Ubicacion = PabellonPorLocal.TryGetValue(local, out var pab) ? pab : "Ubicación no disponible",
                        Fecha = fecha,
                        PuntosLocal = 0,
                        PuntosVisitante = 0
                    });
